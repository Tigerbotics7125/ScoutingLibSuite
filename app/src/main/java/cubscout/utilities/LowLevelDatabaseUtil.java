...
public class LowLevelDatabaseUtil {
  ...
  private static String mUri = "mongodb://localhost:27017";
  private static MongoClient mClient = MongoClients.create(mUri);
  private static CodecProvider pojoCodecProvider = PojoCodecProvider.builder().register("cubscout.backend.pojos")
      .build();
  ...
  public static <T> MongoCollection<T> getCollection(String dbName, String cName, Class<T> clazz) {
    logger.info(String.format("Attempting to access collection '%s' in database '%s'.", cName, dbName));

    MongoDatabase db = getDatabase(dbName);
    MongoCollection<T> col = db.getCollection(cName, clazz);

    logger.info(
        String.format("Collection '%s' in database '%s' accessed.", cName, dbName));
